...
#send email
def create_pdf_rep(advisor_name, cdata, div_table):
    buffer = io.BytesIO()
...
def send_email(recipient_email, advisor_name):
    try:
        ...
        pdf_buffer = create_pdf_rep(advisor_name, cdata, pivot)
...
#tab4 - pdf report and automation
with tab4:
    ...
    if st.button("Generate Advisor PDF Report"):
        pdf_buffer = create_pdf_rep(selected_pdf_advisor, cdata, div_table)
        st.download_button(
    ...
    # currenty running jobs
    st.markdown("### 📋 Active Jobs")
    ajobs = st.session_state.scheduler.get_jobs()
    if ajobs:
        for job in ajobs:
            st.write(
